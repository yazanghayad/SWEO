...
export function useRealtime<T extends Models.Document>({
  ...
  // We try account.get() but silently swallow CORS / network errors
  // that happen in environments where the origin is not registered
  // as an Appwrite platform (e.g. GitHub Codespaces).
  ...
  useEffect(() => {
    ...
    account
      ...
      .catch(() => {
        // No session or CORS error — Realtime won't work, do not retry
