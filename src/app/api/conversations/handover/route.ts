  ...
  if (tenantConfig.webhookUrl) {
    ...
    try {
      const { validateExternalUrl } = await import(
        '@/lib/security/url-validator'
      );
      await validateExternalUrl(tenantConfig.webhookUrl);
      webhookSafe = true;
    } catch (err) {
      logger.error('[handover] Blocked webhook URL (SSRF protection)', { err });
    }
    ...
    if (webhookSafe) {
      fetch(tenantConfig.webhookUrl, {
        ...
        logger.error('[handover] Webhook notification failed', { err });
      });
    }
  }
