...
import {
  ...
  runScenarioAction,
  seedDemoScenariosAction,
  type RunScenarioData
} from '@/features/testing/actions/scenario-crud';
import type { ActionResult } from '@/features/testing/actions/helpers';
import { seedDemoKnowledgeAction } from '@/features/testing/actions/seed-knowledge';
...
export default function TestingPageClient() {
  ...
  const [lastResult, setLastResult] = useState<ActionResult<RunScenarioData> | null>(null);
  ...
  async function handleRun(id: string) {
    ...
    if (res.success && res.data) {
      toast[res.data.passed ? 'success' : 'warning'](
        res.data.passed ? 'Test passed!' : 'Test failed'
      );
  ...
  async function handleSeedDemo() {
    setSeeding(true);
    try {
      // Seed knowledge vectors first (needed for simulations to succeed)
      const knowledgeRes = await seedDemoKnowledgeAction(tenant!.$id);
      if (knowledgeRes.success && (knowledgeRes.data?.copied ?? 0) > 0) {
        toast.success(`Imported ${knowledgeRes.data!.copied} knowledge vectors`);
      }

      // Then seed test scenarios
      const res = await seedDemoScenariosAction(tenant!.$id);
      if (res.success && res.data) {
        toast.success(
          `Created ${res.data.created} demo scenario${res.data.created !== 1 ? 's' : ''}`
        );
        load();
      } else {
        toast.error(res.error ?? 'Failed to seed scenarios');
      }
    } catch (_err) {
      ...
      <div className='flex w-full max-w-[900px] flex-col gap-6'>
        <div className='flex items-center justify-between'>
          <div className='flex items-center gap-2'>
            <h1 className='font-serif text-lg font-light tracking-tight'>Test</h1>
            ...
            <div className='flex items-center justify-between'>
              <div>
                <p className='font-medium'>
                  {lastResult.data.passed ? '✓ Test passed' : '✗ Test failed'}
                </p>
                <p className='text-muted-foreground text-sm'>
                  Resolution: {lastResult.data.actualResolution ? 'Yes' : 'No'}{' '}
                  (expected: {lastResult.data.expectedResolution ? 'Yes' : 'No'}) ·
                  Confidence:{' '}
                  {(lastResult.data.actualAvgConfidence * 100).toFixed(0)}% (min:{' '}
                  {(lastResult.data.expectedMinConfidence * 100).toFixed(0)}
                  %) · Turns: {lastResult.data.turns}
                </p>
              </div>
              <Button
                variant='ghost'
