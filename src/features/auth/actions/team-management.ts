...
export async function ensureTeamAction(
  ...
  try {
    ...
    logger.error('ensureTeamAction error', { err });
    return {
      success: false,
      error: safeError(err, 'Failed to create team')
    };
  }
}
...
export async function inviteTeamMemberAction(
  ...
  // RBAC: requires team.invite permission (admin+)
  const perm = await checkPermission(tenantId, 'team.invite');
  if (!perm.allowed) return { success: false, error: perm.error ?? 'Access denied' };
  ...
  try {
    ...
    logger.error('inviteTeamMemberAction error', { err });
    return {
      success: false,
      error: safeError(err, 'Failed to invite member')
    };
  }
}
...
export async function listTeamMembersAction(tenantId: string): Promise<{
  ...
  try {
    ...
    logger.error('listTeamMembersAction error', { err });
    return {
      success: false,
      error: safeError(err, 'Failed to list members')
    };
  }
}
...
export async function updateMemberRoleAction(
  ...
  // RBAC: requires team.roles permission (admin+)
  const perm = await checkPermission(tenantId, 'team.roles');
  if (!perm.allowed) return { success: false, error: perm.error ?? 'Access denied' };
  ...
  try {
    ...
    logger.error('updateMemberRoleAction error', { err });
    return {
      success: false,
...
export async function removeMemberAction(
  ...
  // RBAC: requires team.remove permission (admin+)
  const perm = await checkPermission(tenantId, 'team.remove');
  if (!perm.allowed) return { success: false, error: perm.error ?? 'Access denied' };
  ...
  try {
    ...
    logger.error('removeMemberAction error', { err });
    return {
      success: false,
      error: safeError(err, 'Failed to remove member')
    };
  }
}
