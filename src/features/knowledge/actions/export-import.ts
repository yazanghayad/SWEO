...
export async function exportKnowledgeBaseAction(tenantId: string): Promise<
  ActionResult<KnowledgeExportManifest>
> {
  try {
    ...
    for (const source of result.documents) {
      ...
      if (source.type === 'file' && source.fileId) {
        try {
          ...
          logger.warn('Could not download file during export', {
    ...
    logger.error('exportKnowledgeBaseAction error', { err });
...
export async function importKnowledgeBaseAction(
  ...
  try {
    ...
    for (const source of manifest.sources) {
      ...
      try {
        ...
        try {
          ...
          }).catch((err) => logger.error('Failed to trigger embedding job after import', { err }));
        }
        ...
        importResult.details.push({
          originalId: source.originalId,
          status: 'failed',
          reason: safeError(err, 'Unknown error')
        });
      }
    }
    ...
    logger.error('importKnowledgeBaseAction error', { err });
    return { success: false, error: toErrorMessage(err) };
  }
}
