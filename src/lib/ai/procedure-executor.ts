...
function evaluateCondition(
  ...
  // If unresolved template variables remain, the condition cannot be evaluated
  if (/\{\{[^}]+\}\}/.test(interpolated)) {
    throw new Error(
      `Unresolved template variables in condition: "${interpolated}"`
    );
  }
...
async function loadConnector(
  ...
  try {
    ...
    // Decrypt credentials so procedure steps can authenticate
    const { decryptCredentials } = await import(
      '@/lib/encryption'
    );
    if (doc.auth?.credentials) {
      try {
        doc.auth = {
          ...doc.auth,
          credentials: decryptCredentials(doc.auth.credentials)
        };
      } catch {
        // Credentials may be stored unencrypted (legacy / dev)
      }
    }
