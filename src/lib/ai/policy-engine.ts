...
/**
 * Reject user-supplied regex patterns that may cause catastrophic backtracking.
 *
 * Checks:
 *  1. Max length 200 chars
 *  2. No consecutive quantifiers like `a**`, `a++`, `a*+`
 *  3. No nested quantifiers like `(a+)+`, `(a*){2,}`, `(a+|b+)*`
 *  4. No more than 3 quantifiers total (limits combinatorial explosion)
 *  5. No backreferences (`\1`, `\2`, …) — they can cause exponential time
 *  6. Pattern must parse as a valid RegExp
 */
function isSafeRegex(pattern: string): boolean {
  // Length limit
  if (pattern.length > 200) return false;

  // Consecutive quantifiers: `**`, `++`, `?+`, `*?*`, etc.
  if (/[*+?]\s*[*+?]/.test(pattern)) return false;

  // Quantifier after a group that itself contains a quantifier or alternation:
  //   (a+)+  (a|b)+  (a*){2}  (?:a+)*  etc.
  if (/\([^)]*[+*?][^)]*\)\s*[+*?{]/.test(pattern)) return false;
  if (/\([^)]*\|[^)]*\)\s*[+*?{]/.test(pattern)) return false;

  // Too many quantifiers in one pattern
  const quantifierCount = (pattern.match(/[+*?]|\{\d/g) ?? []).length;
  if (quantifierCount > 3) return false;
...
function validateTopicFilter(
  ...
  if (config.blockedPatterns) {
    // Limit content length for regex testing to mitigate ReDoS
    const truncated = content.slice(0, 5000);

    for (const pattern of config.blockedPatterns) {
      if (!isSafeRegex(pattern)) continue;
      try {
        if (new RegExp(pattern, 'i').test(truncated)) {
          return {
            policyId: policy.$id,
            policyName: policy.name,
            policyType: 'topic_filter',
