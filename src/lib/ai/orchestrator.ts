...
import { vectorSearch, normalizeScore, type SearchResult } from './retrieval';
...
export interface OrchestratorInput {
  ...
  channel?: 'web' | 'email' | 'whatsapp' | 'sms' | 'voice' | 'instagram' | 'facebook_messenger' | 'slack';
...
/**
 * Compute the best (maximum) score from search results and normalize it.
 *
 * Using the top score as confidence is more accurate than averaging all
 * top-K results, because lower-ranked chunks unfairly drag the average
 * down even when the best result is highly relevant.
 *
 * Normalization maps the raw cosine similarity through a model-aware
 * mapping so that thresholds (like 0.7) are comparable across embedding
 * models (NVIDIA E5 produces lower raw scores than OpenAI).
 */
function bestScore(results: SearchResult[]): number {
  if (results.length === 0) return 0;
  const raw = Math.max(...results.map((r) => r.score));
 ...
 * Also returns tenantName for LLM identity.
 */
async function loadTenantConfig(
  tenantId: string
): Promise<TenantConfig & { _tenantName?: string }> {
  try {
    const { databases } = createAdminClient();
    ...
    const config: TenantConfig & { _tenantName?: string } =
      typeof raw === 'string' ? JSON.parse(raw) : ((raw ?? {}) as TenantConfig);
    config._tenantName = config.botName || (tenant as { name?: string }).name || undefined;
    return config;
  } catch {
    return {};
  }
...
export async function orchestrate(
  ...
  try {
    ...
    // SECURITY: Never silently continue without policies in production.
    // An empty policy set means all pre- and post-checks pass automatically,
    // which would bypass safety enforcement.
    if (process.env.NODE_ENV === 'production') {
      logger.error('Policy load failed — blocking request (fail-closed)', {
        err,
        tenantId
      });
      return {
        resolved: false,
        content:
          'Our safety system is temporarily unavailable. Please try again in a moment.',
        conversationId: conversationId ?? 'unknown',
        messageId: null,
        confidence: 0,
        citations: [],
        blockedReason: 'policy_load_failed',
        escalated: false,
        debug: {
          ...debug,
          durationMs: Date.now() - startTime,
          prePolicyPassed: false,
          prePolicyViolations: ['Policy engine unavailable — fail-closed']
        }
      };
    }
    logger.warn(
      'Policy load failed — continuing without policies (dev only)',
      { err, tenantId }
    );
  }
  ...
  try {
    ...
    if (matchedProcedure) {
      ...
      if (procedureResult.success && procedureResult.finalMessage) {
        ...
        if (!dryRun) {
          await updateConversationStatus(conversationId, 'resolved', tenantId);
        }
    ...
    logger.error('Procedure matching failed, falling back to RAG', { err });
  }
  ...
  debug.avgRetrievalScore = bestScore(retrievalResults);

  // Filter out very low-relevance results so the LLM gets cleaner context.
  // Use normalizeScore to apply a model-aware threshold: only keep results
  // whose normalised confidence is at least 0.15 (maps differently per model).
  retrievalResults = retrievalResults.filter((r) => normalizeScore(r.score) >= 0.15);
  ...
  if (retrievalResults.length === 0 || confidence < CONFIDENCE_THRESHOLD) {
    ...
    if (!dryRun) {
      await updateConversationStatus(conversationId, 'escalated', tenantId);
    }
  ...
  const ragContext: RAGContext = {
    ...
    tenantName: tenantConfig._tenantName
  };
...
async function updateConversationStatus(
  ...
  try {
    ...
    logger.error('Failed to update conversation status', { err });
  }
}
