...
// ---------------------------------------------------------------------------
// Fail-closed helper: reject in production when Redis is unavailable
// ---------------------------------------------------------------------------

/**
 * Check if we are running in a production-like environment.
 * Reads process.env at call time so env stubs in tests work correctly.
 */
...
/**
 * Called when the rate limiter cannot function (Redis not configured or
 * limiter instance unavailable).
 *
 * When Redis is simply not configured we fail **open** — the operator chose
 * not to set up Redis, so we allow requests through with a warning.
 * To enable rate limiting, set UPSTASH_REDIS_REST_URL and
 * UPSTASH_REDIS_REST_TOKEN environment variables.
 */
function failClosed(context: string): RateLimitResult {
  if (isProduction()) {
    logger.warn(
      `[rate-limit] Redis not configured — allowing request (${context}). ` +
        'Set UPSTASH_REDIS_REST_URL and UPSTASH_REDIS_REST_TOKEN to enable rate limiting.'
    );
  } else {
    logger.warn(
      `[rate-limit] Redis not configured — allowing request in dev (${context})`
    );
  }
...
/**
 * Called when a limiter.limit() call throws at runtime (Redis configured but
 * unreachable, network error, etc.).
 *
...
function failClosedOnError(
  ...
  if (isProduction()) {
    logger.error(
      `[rate-limit] Redis error — blocking request (${context})`,
      { err }
    );
    return { success: false, limit: 0, remaining: 0, reset: 0 };
  }
  logger.warn(
    `[rate-limit] Redis error — allowing request in dev (${context})`,
    { err }
  );
  return { success: true, limit: 0, remaining: 0, reset: 0 };
}
...
function getTenantLimiter(plan: string): Ratelimit | null {
  ...
  if (!_tenantLimiters.has(key)) {
    ...
    if (_tenantLimiters.size >= MAX_TENANT_LIMITERS) {
      ...
      if (firstKey) _tenantLimiters.delete(firstKey);
    }
    _tenantLimiters.set(
      key,
      new Ratelimit({
...
export async function checkTenantRateLimit(
  ...
  if (!isRedisConfigured()) {
    return failClosed('tenant');
  }

  const limiter = getTenantLimiter(tenant.plan);
  if (!limiter) {
    return failClosed('tenant-limiter');
  }
  ...
  try {
    ...
    return failClosedOnError('tenant', err);
  }
}
...
export async function checkIpRateLimit(ip: string): Promise<RateLimitResult> {
  if (!isRedisConfigured()) {
    return failClosed('ip');
  }

  const limiter = getIpLimiter();
  if (!limiter) {
    return failClosed('ip-limiter');
  }
  ...
  try {
    ...
    return failClosedOnError('ip', err);
  }
}
...
export async function checkDashboardRateLimit(
  ...
  if (!isRedisConfigured()) {
    return failClosed('dashboard');
  }

  const limiter = getDashboardLimiter();
  if (!limiter) {
    return failClosed('dashboard-limiter');
  }
  ...
  try {
    ...
    return failClosedOnError('dashboard', err);
  }
}
