 ...
 *
 * Handles inbound Facebook Messenger messages via Meta webhook and
 * sends replies through the Messenger Send API.
 *
 * Setup requirements:
 *   1. Meta Developer App with Messenger Product enabled
 *   2. Webhook subscription for `messages` field on `page` object
 *   3. Page access token with `pages_messaging` permission
 */
...
const log = createModuleLogger('messenger-adapter');
...
/** Webhook payload from Meta for Messenger. */
...
export interface MessengerParsedMessage {
  senderId: string;
  recipientId: string;
  messageId: string;
  text: string;
...
export class FacebookMessengerAdapter extends ChannelAdapter {
  readonly channelType: ConversationChannel = 'facebook_messenger';
  ...
  async sendMessage(
    ...
    // Messenger Send API — use Authorization header, never token in URL
  ...
  async receiveMessage(payload: unknown): Promise<IncomingMessage> {
    const data = payload as MessengerParsedMessage & {
      tenantApiKey?: string;
    };

    const tenantApiKey = data.tenantApiKey;
    if (!tenantApiKey) {
      throw new Error('Missing tenantApiKey in Messenger webhook payload');
    }
  ...
  static parseWebhookPayload(
    body: MessengerWebhookPayload
  ): MessengerParsedMessage[] {
    const messages: MessengerParsedMessage[] = [];
    ...
    for (const entry of body.entry) {
      for (const event of entry.messaging) {
        ...
        // Handle postbacks (button clicks) as text
        if (event.postback) {
          messages.push({
            senderId: event.sender.id,
            recipientId: event.recipient.id,
  ...
  private async findOrCreateConversation(
    ...
    const existing = await databases.listDocuments<Conversation>(
      ...
      [
        ...
        Query.equal('channel', 'facebook_messenger'),
...
// Singleton
export const facebookMessengerAdapter = new FacebookMessengerAdapter();
