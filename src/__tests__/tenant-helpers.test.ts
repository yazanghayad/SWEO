...
describe('Tenant Helpers', () => {
  ...
  describe('updateTenantDocument', () => {
    it('verifies tenant ownership then updates the document', async () => {
      adminDatabases.getDocument.mockResolvedValueOnce({
        $id: 'doc-1',
        tenantId: 'tenant-1'
      });

      await updateTenantDocument('policies', 'doc-1', { name: 'Updated' }, 'tenant-1');

      expect(adminDatabases.getDocument).toHaveBeenCalledWith(
        expect.any(String),
        'policies',
        'doc-1'
      );
    ...
    it('throws when document belongs to a different tenant', async () => {
      adminDatabases.getDocument.mockResolvedValueOnce({
        $id: 'doc-1',
      ...
      await expect(
        updateTenantDocument('policies', 'doc-1', { name: 'Updated' }, 'tenant-1')
      ).rejects.toThrow('Forbidden');
      expect(adminDatabases.updateDocument).not.toHaveBeenCalled();
    });
  });
  ...
  describe('deleteTenantDocument', () => {
    it('verifies tenant ownership then deletes the document', async () => {
      adminDatabases.getDocument.mockResolvedValueOnce({
        $id: 'doc-1',
        tenantId: 'tenant-1'
      });

      await deleteTenantDocument('policies', 'doc-1', 'tenant-1');
    ...
    it('throws when document belongs to a different tenant', async () => {
      ...
      await expect(
        deleteTenantDocument('policies', 'doc-1', 'tenant-1')
      ).rejects.toThrow('Forbidden');
      expect(adminDatabases.deleteDocument).not.toHaveBeenCalled();
    });
  });
