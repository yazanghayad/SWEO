...
describe('AI Orchestrator', () => {
  ...
  it('returns result without persisting in dry-run mode', async () => {
    const _adminDb = (createAdminClient as Mock)().databases;
  ...
  // ── Policy load failure → fail-closed in production ────────────────────

  it('blocks request when loadTenantPolicies throws in production', async () => {
    vi.stubEnv('NODE_ENV', 'production');

    (loadTenantPolicies as Mock).mockRejectedValue(
    ...
    try {
      ...
      expect(result.resolved).toBe(false);
      expect(result.blockedReason).toBe('policy_load_failed');
      expect(result.content).toContain('temporarily unavailable');
      // Must NOT have called vector search, LLM, or any downstream step
      expect(vectorSearch).not.toHaveBeenCalled();
      expect(generateCompletion).not.toHaveBeenCalled();
    } finally {
      vi.unstubAllEnvs();
  ...
  it('continues without policies when loadTenantPolicies throws in dev', async () => {
    ...
    try {
      ...
      // In dev, request proceeds — policies are empty but not blocked
      expect(result.resolved).toBe(true);
      expect(result.blockedReason).toBeNull();
    } finally {
      vi.unstubAllEnvs();
